...
const handleClerkWebhook = httpAction(async (ctx, req) => {
  ...
  if (!event) {
    return new Response('Could not verify Clerk payload', { status: 400 });
  }
  ...
  switch (event.type) {
    case 'user.created': {
      ...
      if (user) {
        console.log(`Updaing user ${event.data.id} with: ${event.data}`);
      }
    }

    case 'user.updated': {
      console.log('Creating/Updaing User:', event.data.id);
